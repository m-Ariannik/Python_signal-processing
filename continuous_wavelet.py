'''
This is the built-in cwtm function from the SciPy package. However, it is modified to allow for using db1 wavelet
'''
def cwtm(data, scales, wavelet, sampling_period=1., method='conv', axis=-1):
    ...
    dt_out = dt_cplx if hasattr(wavelet, 'complex_cwt') and wavelet.complex_cwt else dt
    out = np.empty((np.size(scales),) + data.shape, dtype=dt_out)
    precision = 10
    int_psi, x = integrate_wavelet(wavelet, precision=precision)
    int_psi = np.conj(int_psi) if hasattr(wavelet, 'complex_cwt') and wavelet.complex_cwt else int_psi
